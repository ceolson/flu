...
train_sequences_h3_dataset = file['train_sequences_h3']
train_sequences_h3 = train_sequences_h3_dataset[()]
...
noise = tf.placeholder(float,shape=[batch_size,latent_dim])
prediction = tf.nn.softmax(decoder(noise,training=False))
class_prediction = tf.nn.softmax(predictor(prediction))

seq_in = tf.placeholder(float)
test_prediction = tf.nn.softmax(predictor(seq_in))
...
saver_vae.restore(sess,'/home/ceolson0/Documents/flu/models/saves/vae_fc_vae_beta/')
saver_predictor.restore(sess,'/home/ceolson0/Documents/flu/models/saves/vae_fc_predictor/')

# ~ error_count = 0
# ~ frequencies_true = [0 for i in range(19)]
# ~ frequencies_predicted = [0 for i in range(19)]
# ~ for i in range(100):
    # ~ random_permutation = np.random.permutation(range(len(train_sequences)))[:batch_size]
    # ~ preds = sess.run(test_prediction,feed_dict={seq_in:train_sequences[random_permutation]})
    # ~ preds = np.argmax(preds,axis=-1)
    # ~ for i in range(len(preds)):
        # ~ if preds[i] != np.argmax(train_labels[random_permutation][i]):
            # ~ error_count += 1
        # ~ frequencies_predicted[preds[i]] += 1
        # ~ frequencies_true[np.argmax(train_labels[random_permutation][i])] += 1
        
# ~ print('Accuracy:', (100*batch_size-error_count)/(100*batch_size))
# ~ print(frequencies_true)
# ~ print(frequencies_predicted)


samples = []
for i in range(20):
    samples.append(np.random.uniform(low=-3.,high=3.,size=(batch_size,latent_dim)))

samples = np.array(samples)
samples_flat = samples.reshape(20*batch_size,latent_dim)
classes_flat = []

for sample in samples:
    classes = sess.run(class_prediction,feed_dict={noise:sample})
    for c in classes:
        classes_flat.append(np.argmax(c))

classes_flat = np.array(classes_flat)
np.savetxt('classes.txt',classes_flat)
np.savetxt('samples.txt',samples_flat)
